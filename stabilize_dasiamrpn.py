#!/usr/bin/env python3
"""
PoC 2: DaSiamRPN (Distractor-Aware Siamese Region Proposal Network) tracker.

Uses OpenCV's built-in TrackerDaSiamRPN with ONNX models.
Adds: scale constraints, confidence-based re-acquisition, velocity prediction,
histogram appearance validation.

Usage:
    python stabilize_dasiamrpn.py [--source 0] [--padding 80]
    ...
    if original_hist is not None:
        ...
        if curr_hist is not None:
            ...
            if sim < HIST_SIM_THRESHOLD:
                return False, f"appearance(sim={sim:.2f})"
...
def main():
    parser = argparse.ArgumentParser(description='DaSiamRPN ROI Stabilizer')
    ...
    win_crop = 'Stabilized Crop (DaSiamRPN)'
    ...
    print('[DaSiamRPN] Draw a box around the ROI. Press r to re-select, q to quit.')
    ...
    while True:
        ...
        if state == 'tracking' and tracker is not None:
            ...
            if ok and score > SCORE_LOST:
                ...
                if crop is not None and crop.size > 0:
                    ...
                    cv2.putText(crop_display, 'DaSiamRPN', (10, 55),
        ...
        if state == 'lost' and last_good_bbox is not None:
            ...
            cv2.putText(display, f'LOST ({lost_frame_count}f)', (10, 30),
